...
#include "MySensitiveDetector.hh"  // Include the full header here
#include "G4Track.hh"
...
FlagForRealSignal(0),
SecondaryNameContains("Sc48"),sensitiveDetector(nullptr)
{
...
void MuStoppedTrackingAction::PreUserTrackingAction(const G4Track* aTrack )
{
    ...
    // Call InitializeNewTrack() in my sensitive detector when a new track starts
    if (sensitiveDetector)
    {
        sensitiveDetector->InitializeNewTrack();
    }
    else
    {
        G4cerr << "Error: Sensitive detector is not set in MuStoppedTrackingAction!" << G4endl;
    }
...
void MuStoppedTrackingAction::SetSensitiveDetector(MySensitiveDetector* detector)
{
    // Set the sensitive detector pointer
